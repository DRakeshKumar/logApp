 ...
 /* var groupedServicesTypeList = _.groupBy(xl_data, function (item) { return item['Services Type']});
  ...
  db.property.findOne({"user_id":req.session.user},function (err, data) {
    if(err){
      ...
      //db.services.find({property_id:data.property_id}).sort({ created_on : -1 }).limit(1).toArray(function (err, find_last_data) {
      db.services.find({property_id:data.property_id}).toArray(function (err, find_last_data) {
        if(err){
          return err;
        }else{
          if (find_last_data.length != 0) {
            var d = find_last_data[0].request_id;
            //console.log('d>>>>'+ _.split(d ,"-")[2])
            // var dSplit = _.replace(d,data.property_id+"-RT","");
           // var dSplit = _.replace(_.split(d ,"-")[2],"RT","");
           // console.log('dSplit>>>>'+dSplit)
             newTicketId =  parseInt((find_last_data.length));
            indexCount++;
                ...
                if(arrayOfObjAfter[key]['id']){
                  ...
                  if(_.includes(JSON.stringify(xl_data_header[0]), 'Flat Id')){
                    ...
                    var inserted_json = {
                      ...
                      request_id              :  "RT"+((newTicketId || 0)  + rtCount),
