    ...
    for (var key in xl_data) {
      if (xl_data.hasOwnProperty(key)) {
        if (xl_data[key]['id']) {
          ...
          /*db.property.find({property_id: 'P-3'}).toArray(function (err, property_data) {
            if (err) {
              return err;
            } else {
              if (xl_data_header[0].length > 6 || xl_data_header[0].length < 6) {
              ...
              for (var phase in groupedPhaseList) {
                ...
                for (var tower in groupedTowerList) {
                  ...
                  console.log('groupedFlatList::::::' + JSON.stringify(groupedFlatList));
                  for (var flat in groupedFlatList) {
                    ...
                    flatList.push({
                      ...
                      // flat_id: xl_data[key]['Flat Id'],
                      room: groupedFlatList[flat][0].Rooms
                    })
                  }
                }
              }
              db.property.update({property_id: 'P-3'}, {
                $set: {
              ...
              // console.log('phase::::::'+JSON.stringify(phaseList));
              // console.log('tower::::::'+JSON.stringify(towerList));
              // console.log('flat::::::'+JSON.stringify(flatList));
            }
